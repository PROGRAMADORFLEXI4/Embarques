<?PHP
	...
	else
	{
		...
		else
		{
			...
			$sql=odbc_exec($conn,"SELECT C6_TES,C6_ITEM,C6_PRODUTO,B1_DESC,B1_UBIC,C6_QTDVEN-C6_QTDENT AS C6_QTDVEN,C6_PRCVEN C6_PRUNIT,C6_VALOR,C6_PEDIM,B1_QE,B1_CANTBOL,C6_LOCAL
				FROM SC6010 SC6 
				INNER JOIN SB1010 SB1 ON B1_COD=C6_PRODUTO 
				INNER JOIN(SELECT C9_PRODUTO,C9_ITEM FROM SC9010 WHERE C9_PEDIDO='".$_POST["txtPed"]."' AND C9_BLCRED<>'09' AND C9_BLCRED<>'10' AND D_E_L_E_T_='' 
				...
				$pedido=odbc_fetch_array($ped);
				 echo "<html>
						...
						<div class='page'>
							...
							echo "
							<tr><th colspan='7' style='text-align:center;font-size:15px;font-weight:700;'><h1>".$leyendatxt."</h1></th></tr>
							<tr><th colspan='7' style='text-align:left'>Pedido: ".$_POST["txtPed"]." Cliente: ".$pedido["C5_CLIENTE"]." ".trim($pedido["A1_NOME"]).", Estado: ".$pedido["est"].", Vendedor: ".$pedido["A3_NOME"].", Tipo: ".$tipocte."</th></tr>
							<tr><td colspan='7'><hr></td></tr>
							<tr class='trEnc'><td>Item</td><td>Producto</td><td>Descripci&oacute;n</td><td>TES</td><td>Almacén</td><td>P. Unit</td><td>Cantidad</td><td>Cajas</td><td>Valor Total</td><td>Ubicaci&oacute;n</td></tr><tr><td colspan='8'><hr></td></tr>";
						 while($datos=odbc_fetch_array($sql)){
							 ...
							 echo "<tr><td>".$datos["C6_ITEM"]."</td><td>".trim($datos["C6_PRODUTO"])."</td><td>".trim($datos["B1_DESC"])."</td><td>".trim($datos["C6_TES"])."</td><td class='tdD'>".trim($datos["C6_LOCAL"])."</td><td class='tdD'>$".number_format($datos["C6_PRUNIT"],2)."</td><td class='tdD'>".number_format($datos["C6_QTDVEN"],0)."</td><td class='tdD'>".($cajas>0?$cajas."C ":"").($bolsas>0?$bolsas."B ":"").($resto>0?$resto."R ":"")."</td><td class='tdD'>$".number_format($sub,2)."</td><td class='tdD'>".trim($datos["B1_UBIC"])."</td></tr>";
						 }
						 odbc_free_result($sql);
