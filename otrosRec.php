  ...
  <title>Otras Rutas</title>
	...
	<script>
		$(document).on('ready', function(){
			...
			$(".mpS, #fN").on("click", function(){$("#dvSA").slideUp(100);$("#fN").hide();$("#detM").hide();$("#detM2").hide();});
			...
			$(".tdCod,.mSol").on("click",function(){
				$.post("sm.php",{np:"td"+$(this).attr("id")},function(data,status){$("#detM").html(data);
				 ...
				 $.post("sm2.php",{np:"td"+$(this).attr("id")},function(data,status){$("#detM").html(data);
					if(status="success"){
					  var objeto=document.getElementById('detM'); 
					  var ventana=window.open('','_blank');
					  ventana.document.write(objeto.innerHTML);
			...
			$("#clsSM").live("click",function(){$("#detM").hide();$("#detM2").hide();});
		});
	</script>
...
try{
	...
	if(isset($_GET["salm"])){
		echo "<section><h2>MUESTRAS PENDIENTES POR SURTIR.</h2><article id='detM'></article><article id='detM2'></article>
				<table class='tSAL'>
