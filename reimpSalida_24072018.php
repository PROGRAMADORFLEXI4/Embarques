		  ...
		  $fechaSal=$datos['ZZS_FYHSAL'];
	  	  echo "<body onload=imprimeSalida('".$_GET["Ped"]."','".$alm."')>		  
					...
					while($impreso<2)
					{
						...
						$multiBox=0;
						$partida=0;
						$sql=odbc_exec($conn,"SELECT C5_DIREMB,C5_NUM,C5_EMISSAO,A1_COD,A1_NOME,C5_FYHSURT FROM SC5010 SC5 INNER JOIN SA1010 SA1 ON C5_CLIENTE=A1_COD WHERE C5_NUM='".$_GET["Ped"]."' AND SC5.D_E_L_E_T_='' AND SA1.D_E_L_E_T_=''")
