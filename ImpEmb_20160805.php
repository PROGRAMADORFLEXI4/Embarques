<?php
	...
	elseif(trim(isset($_GET['fac2']))<>""){
		...
		{
			$sql=odbc_exec($conn, "SELECT ZZO_PEDIDO,A1_COD,A2_NOME,A1_NOME,A1_END,A1_BAIRRO,A1_CEP,A1_MUN+', '+A1_EST AS 'poblacion','('+A1_DDD+')'+A1_TEL AS 'telefono',A1_CGC,SUM(ZZM_CAJAS) AS 'cajas',SUM(ZZM_COSTAL) AS 'costales',SUM(ZZM_EXHIB) AS 'exhibidores',SUM(ZZM_TINACO) AS 'tinacos' FROM ZZO010 ZZO LEFT JOIN SA1010 SA1 ON ZZO_CLTE=A1_COD LEFT JOIN ZZM010 ZZM ON ZZO_FACT=ZZM_FATURA INNER JOIN SA2010 SA2 ON ZZO_CODFLE=A2_COD WHERE ZZO.R_E_C_N_O_=".$_GET['fac2']." AND ZZO.D_E_L_E_T_='' AND SA1.D_E_L_E_T_='' AND ZZM.D_E_L_E_T_='' AND SA2.D_E_L_E_T_='' GROUP BY A1_NOME,A1_END,A1_BAIRRO,A1_CEP,A1_MUN,A1_EST,A1_DDD,A1_TEL,A1_CGC,A2_NOME,ZZO_PEDIDO,A1_COD")
				...
				<body class='body'>
					<div class='page1'>
						<div class='sobre'>";
							...
							echo "
							<center> Fleximatic S.A. de C.V.<br>
					 	    RFC: FLE980113E95<br>
							Camino Real de Colima #901-14<br>
							CP: 45645   Tel:(33)3540-1050<br>
							Tlajomulco de Zuñiga, Jal. Mex.
							</center>
						</div>
						<div class='sCpo'>
						...
						<div class='pieP'>CHUBB DE MEXICO SA DE CV. NO. POLIZA : TR 43202153-2<br><br>
						   <label class='lblTit'>Cajas:</label><label class='lblTit'>Costales:</label><label class='lblTit'>Exhibidores:</label><label class='lblTit'>Tindec:</label><br>
						   <label class='lblTit'>Transporte:</label><label class='lblTit'>Cobro:</label>
						   ...
						   <label class='lblInfo'>".number_format($datos['tinacos'],0)."</label><br>
						   <label class='lblInfo'>".trim($datos['A2_NOME'])."</label><label class='lblInfo'>".$_GET['pag']."</label></div><div class='pP'>";
						   if(trim($_GET['obs'])<>'')
	...
	elseif(isset($_GET["Act"])<>""){
		...
		while($registros<>"")
		{
			...
			$datos=odbc_fetch_array($sql);
			odbc_free_result($sql);
			odbc_exec($conn,"UPDATE ZZO010 SET ZZO_CHOFER='$chofer',ZZO_CONSEC=".$datos['consec']." WHERE R_E_C_N_O_=$reg")or die("Error al actualizar los registros de embarque");
		}
	}
	else
	{
		//Cancela factura de ruta
		odbc_exec($conn,"UPDATE ZZO010 SET ZZO_CHOFER='',ZZO_CONSEC=0 WHERE R_E_C_N_O_=".$_GET['reg']) or die("Error al actualizar los registros de embarque");
	}
	odbc_close($conn);
?>