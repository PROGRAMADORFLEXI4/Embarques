<html>
	...
	<title>Salida por Pedido</title>
		...
		<script>
			function imprimeSalida(ped,alm,sal,ord_sur){
				...
				//location.href="guardaSal.php?Ped="+ped+"&Alm="+alm+"&Caj="+cajas+"&Cos="+costal+"&Exh="+exhibidor+"&Tin="+tindec+"&Sal="+sal;
				
				console.log("pedido: " + ped + " almacenista: " + alm + " cajas: " + cajas + " costales: " + costal + " exhibidor: " + exhibidor + " tinacos: " + tindec + " Salida: " + sal + " ord_sur " + ord_sur);

				$.ajax({
					url: 'guardaSal.php',
					type: 'post',
					data: {pedido: ped, almacenista: alm, cajas: cajas, costales: costal, exhi: exhibidor, tindec: tindec, salida: sal, ordsur: ord_sur},
					success: function (res) {
						...
						if (res.trim() == "GOOD") {
							...
							alert("Ocurrio un error al guardar el resumen del pedido");
							setTimeout(function () { 
								window.open('index.php', '_self');
	 						}, 1000);
						}
				...
				/*var _link = "pedido_salida.php?pedido="+ped+"&salida="+sal;
...
<?php
	echo "<body onload=imprimeSalida('".$_GET["Ped"]."','".$_GET["Alm"]."','".$_GET["Sal"]."','".$_GET["ordsur"]."')>
		<form><center>";
		...
		while($impreso<2)
		{
			...
			$multiBox=0;
			$partida=0;
			$sql=odbc_exec($conn,"SELECT C5_DIREMB,C5_NUM,C5_EMISSAO,A1_COD,A1_NOME,C5_FYHSURT FROM SC5010 SC5 INNER JOIN SA1010 SA1 ON C5_CLIENTE=A1_COD WHERE C5_NUM='".$_GET["Ped"]."' AND C5_NOTA='' AND SC5.D_E_L_E_T_='' AND SA1.D_E_L_E_T_=''")
						  ...
						  while($datos=odbc_fetch_array($sql))
						  {
							...
							if($inner>0 && $cantidad==0 && $datos["B1_CANTBOL"]>0)
							{
								...
								/*echo "
									...
									<td align='right' ".$borde.">".intval($datos["ZDS_QTENT"])."</td>
									<td ".$borde.">[".trim($datos["B1_COD"])."] ".trim($datos["B1_DESC"])."</td>
									<td ".$borde.">".trim($datos["ZDS_PEDIM"])." ".$inner." bolsas ".($innerres>0?$innerres?"")."&nbsp;</td>";*/
							}
							else
							{
								$items="";
							...
							if($partida<>$datos["ZDS_PARTID"] || $partida==0)
							{
								...
								{
									$costal+=$cantidad;
									echo "<td ".$borde.">&nbsp;</td><td style='border:solid 1px #D8D8D8;'>&nbsp;</td>";
								}
								else
								{
									$caja+=$cantidad;
