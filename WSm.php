<?php
	...
	$IMEI="357344060374932"; 
	//LUIS HTC 357344060374932
	//rafael carlos 867530020454953
	//$CODIGOFACTURA=$_POST['CF'];
	$CODIGOFACTURA="00000000000000091882";
	//$GUIA=$_POST['G'];
	//$GUIA="Gdl 768";
	$GUIA="LOCAL";
	//$VALORGUIA=$_POST['VG'];
	...
	$FECHAENTREGA="11/05/2015 13:04:48";
	$Enviar = 0;
		...
		if($GUIA == "LOCAL" || $GUIA == "local"){
			...
			if($reg=odbc_fetch_array($registros)){
				...
				while($reg=odbc_fetch_array($Pedidos)){
					...
					echo $FECHADIV;
					//fwrite($file, date('Ymd H:i:s')." Actualizando datos para factura ".$CODIGOFACTURA." GUIA: ".$GUIA." Valor Guia: ".$VALORGUIA." Fecha Entrega: ".$FECHAENTREGA." OBS: ".$OBS.PHP_EOL);
					//odbc_exec($conn,"UPDATE ZZO010 set ZZO_GUIA='$GUIA', ZZO_VALORG='$VALORGUIA',ZZO_OBSERV='$OBS',ZZO_FECHEN='$FECHAENTREGA', ZZO_FECENT='$FECHADIV' WHERE ZZO_FACT='$FACTURA'") or die("Problemas en el select");
					echo "aqui";
				}
			}
		}
			...
			//fwrite($file, date('Ymd H:i:s')." Actualizando datos para factura ".$CODIGOFACTURA." GUIA: ".$GUIA." Valor Guia: ".$VALORGUIA." Fecha Entrega: ".$FECHAENTREGA." OBS: ".$OBS.PHP_EOL);
			//odbc_exec($conn,"UPDATE ZZO010 set ZZO_GUIA='$GUIA', ZZO_VALORG='$VALORGUIA',ZZO_OBSERV='$OBS',ZZO_FECHEN='$FECHAENTREGA' WHERE ZZO_FACT='$CODIGOFACTURA'") or die("Problemas en el select");
			$Enviar = 1;
		}
	}
	else{
		fwrite($file, date('Ymd H:i:s')." Obtenienfo ruta para  ".$IMEI. PHP_EOL);
		$registros=odbc_exec($conn,"SELECT ZZO_FECHEN,ZZN_NOMBRE,ZZO_FACT,A1_NOME,ISNULL(A2_NOME,'LOCAL') AS A2_NOME,ZZM_CAJAS,ZZM_COSTAL,ZZM_EXHIB,ZZM_TINACO,ZZO_GUIA FROM ZZO010 ZZO INNER JOIN SA1010 SA1 ON ZZO_CLTE=A1_COD LEFT JOIN SA2010 SA2 ON ZZO_CODFLE=A2_COD INNER JOIN ZZN010 ZZN ON ZZO_CHOFER=ZZN_CODIGO LEFT JOIN ZZM010 ZZM ON ZZO_FACT=ZZM_FATURA WHERE ZZN_IMEI='$IMEI' AND (ZZO_FECHEN='' OR LTRIM(RTRIM(ZZO_GUIA))='PENDIENTE') AND ZZO.R_E_C_N_O_ > 26874 AND ZZO.D_E_L_E_T_='' AND SA1.D_E_L_E_T_='' AND SA2.D_E_L_E_T_='' AND ZZN.D_E_L_E_T_='' AND ZZM.D_E_L_E_T_='' ORDER BY ZZO_CONSEC") or die("Problemas en el select");
	...
	/*if($Enviar == 1){
		$Enviar = 0;
		$registros_email=odbc_exec($conn,"SELECT A1_NOME,A1_EMAIL,A3_EMAIL,ZZO_FACT,ZZO_PEDIDO,A2_NOME,ZZO_GUIA,ZZO_FECHEN,ZZM_CAJAS,ZZM_COSTAL,ZZM_EXHIB,ZZM_TINACO FROM ZZO010 INNER JOIN SA1010 SA1 ON ZZO_CLTE=A1_COD LEFT JOIN ZZM010 ON ZZO_FACT=ZZM_FATURA LEFT JOIN SA2010 SA2 ON ZZO_CODFLE=A2_COD LEFT JOIN SA3010 SA3 ON A1_VEND=A3_COD WHERE ZZO_FACT='$CODIGOFACTURA' AND SA3.D_E_L_E_T_='' AND SA1.D_E_L_E_T_=''") or die("Problemas en el select");
		if($reg=odbc_fetch_array($registros_email)){
			...
			if($IMEI=="358467066146829")
			{
				$mail->AddAddress("lesparza@fleximatic.com.mx");
			...
			else
			{
				...
				{
					$mail->AddAddress($email_cliente[$pos]);
				}
				$mail->AddCC($email_vendedor);
				$mail->AddBCC("lesparza@fleximatic.com.mx");
				$mail->AddBCC("embarques@fleximatic.com.mx");
			}
			$mail->Subject = "Fleximatic: Informe de entrega del pedido";
			$mail->AddEmbeddedImage('images/logo.png','logo','logo.png');
			$mail->AddEmbeddedImage('images/cintillo.png','cintillo','cintillo.png');
			...
			$body = "<head>
					...
					<body>
							<div id='fuenteTexto1' class='alinearTexto'>
								<b> DE: </b> FLEXIMATIC SA DE CV [info@fleximatic.com.mx]<br>
								<b> ENVIADO: </b> $fecha_hora<br>
								<b> PARA: </b> $nombre_cliente <br>
								<b> ASUNTO: </b> Notificación de Pedido <br><br>
								<IMG SRC='cid:logo' WIDTH=158 HEIGHT=60 BORDER=0 ALT='FLEXIMATIC' /> <br> <br>
									...
									if($impreso < 1)
									{
										...
										if(odbc_num_rows($sql1)>0)
										{
											...
											$body=$body. "<table border='1' class='salidatable'>
												<tr>
													  <td rowspan='2' align='center'><IMG SRC='cid:logo' BORDER=0 ALT='FLEXIMATIC' height='40' width='80'></td>
													  <td align='center' colspan='2' style='font-family:Calibri;font-size:8pt;'><strong>Fleximatic S.A. de C.V.</strong></td>
			...
			if(!$mail->Send()) 
			{
				...
				$mail->From = $resC['COR_USER'];
				$mail->AddAddress("lesparza@fleximatic.com.mx");
				$mail->AddAddress("creditoycobranza@fleximatic.com.mx");
				$mail->IsHTML(true);  
				$mail->Subject = "Fleximatic: ERROR en Informe de entrega del pedido";
				$mail->AddEmbeddedImage('images/logo.png','logo','logo.png');
				$mail->AddEmbeddedImage('images/cintillo.png','cintillo','cintillo.png');
				$nombre_cliente = utf8_decode($nombre_cliente);
				$nombre_fletera = utf8_decode($nombre_fletera);
