		...
		$(document).on("ready",function(){
			...
			$("#asChof").live("click", function(){
				...
				for(i=1;i<controles.length;i++)
				{
					if(controles.elements[i].value!='' && controles.elements[i].type!="button"){
						actReg+=controles.elements[i].value+","+controles.elements[i].name.substr(7)+"|";
						$("#tr"+controles.elements[i].name.substr(7)).remove();
						i--;
					}
				}
				if(actReg!="")
					$.get("impEmb.php",{Act:actReg},function(data){mPie(); });
			});
			...
			$("a.fxe, #btnIm").live("click", function(){
				...
				if($("#cmbF"+comp).attr("value")=="")
					...
					$("#btnCanc").hide();
				    $("#btnIm").hide();
					$.get("impEmb.php",{fac2:comp,obs:$("#txtObs"+comp).attr("value"),pag:$("#cmbC"+comp).attr("value"),flet:$("#cmbF"+comp).attr("value"),suc:$("#cmbDE"+comp).attr("value"),client:$("#txtCli"+comp).attr("value")},function(data,status){$("#prntInfo").html(data);
						if(status="success"){
			...
			$("#qRuta").live("click", function(){$.get("impEmb.php",{reg:$(this).attr("class")}); $("#li"+$(this).attr("class")).remove(); mCuerpo();});
        ...
        <table>
            <tr>
                ...
                <td class="exportacion">Exportaci&oacute;n</td><td class="especiales">Especiales</td><td class="programados">Programados</td>
               	<td class="cita">Cita</td><td class="aclaracion">Aclaraciones</td><td class="bko">BackOrder</td><td class="publicidad">Publicidad</td>
            </tr>
        </table>
    </div>    
	<div class="divs" id="encabezado"></div>
