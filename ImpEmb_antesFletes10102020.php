<?php
	...
	elseif(trim(isset($_GET['fac2']))<>"")
	{
		...
		else
		{
			...
			$sql=odbc_exec($conn, "SELECT C5_FYHSURT,ZZO_CODFLE,ZZO_FECFAC,ZZO_FECFAC,ZZO_FACT,ZZO_PEDIDO,A1_COD,A2_NOME,A1_NOME,A1_END,A1_BAIRRO,A1_CEP,ltrim(rtrim(A1_MUN))+', '+LTRIM(RTRIM(X5_DESCSPA)) AS 'poblacion','('+A1_DDD+')'+A1_TEL AS 'telefono',A1_CGC,SUM(ZZM_CAJAS) AS 'cajas',SUM(ZZM_COSTAL) AS 'costales',SUM(ZZM_EXHIB) AS 'exhibidores',SUM(ZZM_TINACO) AS 'tinacos' FROM ZZO010 ZZO LEFT JOIN SA1010 SA1 ON ZZO_CLTE=A1_COD 
				LEFT JOIN ZZM010 ZZM ON ZZO_FACT=ZZM_FATURA 
				LEFT JOIN SC5010 C5  ON ZZO_PEDIDO=C5_NUM
				INNER JOIN SA2010 SA2 ON ZZO_CODFLE=A2_COD INNER JOIN SX5010 SX5 ON X5_TABELA='12' AND X5_CHAVE=A1_EST AND SX5.D_E_L_E_T_='' WHERE ZZO.R_E_C_N_O_=".$_GET['fac2']." AND ZZO.D_E_L_E_T_='' AND SA1.D_E_L_E_T_='' AND ZZM.D_E_L_E_T_='' AND SA2.D_E_L_E_T_='' GROUP BY ZZO_CODFLE,ZZO_FECFAC,A1_NOME,A1_END,A1_BAIRRO,A1_CEP,A1_MUN,X5_DESCSPA,A1_DDD,A1_TEL,A1_CGC,A2_NOME,ZZO_PEDIDO,A1_COD,C5_FYHSURT,ZZO_FACT")
