<?PHP
	...
	$sql=odbc_exec($conn,"SELECT ZZS_SALIDA AS max FROM ZZS010 WHERE ZZS_PEDIDO='".$_GET["Ped"]."' AND ZZS_CODALM='".$_GET["Alm"]."' AND ZZS_FAC2='F' AND D_E_L_E_T_=''")
	or die("Error al consultar si se encuentra la salida registrada");
	if(odbc_num_rows($sql)>0)
	{
		$salida=odbc_fetch_array($sql);
		odbc_exec($conn,"UPDATE ZZS010 SET ZZS_FECHA='".date("Ymd")."',ZZS_FYHSAL='".date("d/m/y H:i:s",time())."' WHERE ZZS_SALIDA=".intval($salida["max"]))
	...
	else
	{
		...
		$salida=odbc_fetch_array($sql);	
		odbc_free_result($sql);
		odbc_exec($conn,"INSERT INTO ZZS010(ZZS_FILIAL,ZZS_SALIDA,ZZS_PEDIDO,ZZS_FECHA,ZZS_CLIENT,ZZS_CODALM,ZZS_FYHSAL,D_E_L_E_T_,R_E_C_N_O_,ZZS_FAC2) VALUES('',".intval($salida["max"]).",'".$_GET["Ped"]."','".date("Ymd")."','".$_GET["Cli"]."','".$_GET["Alm"]."','".date("d/m/Y H:i:s",time())."','',".intval($salida["max"]).",'F')")
	...
	echo "<script languaje='JavaScript'>
			location.href='impSal.php?Ped=".$_GET["Ped"]."&Alm=".$_GET["Alm"]."&Sal=".intval($salida["max"])."';
		  </script>";
?>