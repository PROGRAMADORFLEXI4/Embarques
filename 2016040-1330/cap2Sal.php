  ...
  <head>
  	<script language="javascript">
		function actualizaQE(campo,qe)
		{
			...
			location.href="guardaTempo.php?Ped="+document.getElementById("txtPedi2").value+"&cant="+campo.value+"&cod="+nCampo+"&qe="+qe;
		}
		...
		function guardaPed()
		{
			...
			if(codigos!="") controles.elements[i-1].value
				...
				else
				{
					if(parseInt(campo.value)!=parseInt(modifica2.value))
					{
						...
						if(campo.id.indexOf("-F")>0 || campo.id.indexOf("-T")>0)
							location.href="guardaTempo.php?Ped="+pedido+"&opc=U&cod="+tempo+"&cant="+parseInt(campo.value)+"&sol="+parseInt(solicita2.value);
						else
							location.href="guardaTempo.php?Ped="+pedido+"&opc=O&cod="+tempo+"&cant="+parseInt(campo.value)+"&sol="+parseInt(solicita2.value);
					}
				}
			}
		}
...
<body>
    ...
    <?php
		...
		or die("Error al validar el pedido en SC5");
		...
		$sql=odbc_exec($conn,"SELECT Id,Producto AS C6_PRODUTO,Pedimento AS C6_PEDIM,B1_DESC,B1_TIPO,B1_CLASE,Empaque,Qe AS costal,CASE WHEN Qe=0 THEN 1 ELSE Qe END AS B1_QE,Cant AS cantR,SUM(C6_QTDVEN-C6_QTDENT) AS Solicitado FROM tempo LEFT JOIN SC6010 SC6 ON producto=C6_PRODUTO INNER JOIN SB1010 SB1 ON B1_COD=Producto LEFT JOIN SC9010 SC9 ON C6_NUM=C9_PEDIDO WHERE Pedido='".$_GET["Ped"]."' AND C6_NUM='".$_GET["Ped"]."' AND C6_PRODUTO=C9_PRODUTO AND C6_ITEM=C9_ITEM AND C9_BLCRED<>'09' AND SB1.D_E_L_E_T_<>'*' AND SC6.D_E_L_E_T_='' AND SC9.D_E_L_E_T_='' GROUP BY Id,Producto,Pedimento,B1_DESC,B1_TIPO,B1_CLASE,Empaque,Qe,Cant ORDER BY B1_QE,Producto")or die("Error al obtener las partidas del pedido SC6");
		if(odbc_num_rows($sql)>0)
		{
			echo "<tr>
			        <td colspan='4'>
						...
						else
						{
							...
							if(intval($datos["cantR"]/$datos["B1_QE"])<>ceil($datos["Solicitado"]/$datos["B1_QE"]))
							{
								...
								if(trim($empaknTindec)<>"")
									echo $empaknTindec;
								echo "<input type='text' id='txtSurR".$datos["Id"]."' value='".intval($cantidad*$datos["B1_QE"])."' size='3' maxlength='5' style='text-align:right' onkeypress='soloNumeros(this)' onblur=mayorSolicitado(this,'".$_GET["Ped"]."')>";
								if($datos["B1_TIPO"]=="04")
								{
									if($datos["Empaque"]=="T")
								...
								echo " [PxE]:<input type='text'";									
									...
									echo "<input type='hidden'";
										echo " id='txtQE".$datos["Id"]."' value='".intval($datos["B1_QE"])."' size='3' maxlength='5' onkeypress='soloNumeros(this)' style='text-align:right' style='background-color:#CCC' onchange='actualizaQE(this,1)'></td>
										<td><font size='-1'>[".trim($datos["C6_PRODUTO"])."] ".trim($datos["B1_DESC"])."</font></td>
										<td><input type='hidden' value='".trim($datos["C6_PEDIM"])."' id='txtPedim".$datos["Id"]."'><font size='-1'>".trim($datos["C6_PEDIM"])."</font></td>";
							...
							else
							{
								...
								if($datos["B1_TIPO"]==01 && $datos["B1_CLASE"]==11)
								{
									...
									while($cuantos>0)
									{
										if($bgColor=="#FFFFFF")
											...
											$bgColor="#FFFFFF";
										echo "<tr bgColor='".$bgColor."'>
												<td align='center'><input type='hidden' id='txtReg".$datos["Id"]."-T' value='".$registros."'><input type='hidden' id='txtSurS".$datos["Id"]."-T' value='".$datos['Solicitado']."'><input type='hidden' id='txtMod".$datos["Id"]."-T' value='1'>[".$registros."]<input type='text' id='txtSurR".$datos["Id"]."-T' value='1' size='3' maxlength='5' style='text-align:right' onkeypress='soloNumeros(this)' onblur=mayorSolicitado(this,'".$_GET["Ped"]."')><input type='hidden' id='txtQE".$datos["Id"]."-T' value='".intval($datos["B1_QE"])."' size='3' maxlength='5' onkeypress='soloNumeros(this)' style='text-align:right' style='background-color:#CCC' onchange='actualizaQE(this,1)'></td>
									<td><font size='-1'>[".trim($datos["C6_PRODUTO"])."] ".trim($datos["B1_DESC"])."</font></td>
									<td><input type='hidden' value='".trim($datos["C6_PEDIM"])."' id='txtPedim".$datos["Id"]."-T'><font size='-1'>".trim($datos["C6_PEDIM"])."</font></td><td align='center'><input type='text' disabled style='text-align:right' style='background-color:".$bgColor."' value='1' size='2' id='txtEmp".$datos["Id"]."-T'></td></tr>";												
									if($cuantos>1)
										$registros++;
									$cuantos--;
									}
								...
								{
							echo "<tr bgColor='".$bgColor."'>
									<td align='center'><input type='hidden' id='txtReg".$datos["Id"]."' value='".$registros."'><input type='hidden' id='txtSurS".$datos["Id"]."' value='".round($datos["Solicitado"])."'><input type='hidden' id='txtMod".$datos["Id"]."' value='".round($datos["cantR"])."'>[".$registros."] ";
							/*Si hay tinacos, agrega el combo*/
							if(trim($empaknTindec)<>"")
								echo $empaknTindec;									
							echo "<input type='text' id='txtSurR".$datos["Id"]."' value='".round($datos["cantR"])."' size='3' maxlength='5' style='text-align:right' onkeypress='soloNumeros(this)' onblur=mayorSolicitado(this,'".$_GET["Ped"]."')>";
							if($datos["B1_TIPO"]=="04")
							{
								if($datos["Empaque"]=="T")
									echo " Costal<input type='radio' value='T' id='rdb".$datos["Id"]."' checked onclick='actualizaQE(this,2)'> Caja<input type='radio' value='F' id='rdb".$datos["Id"]."' onclick='actualizaQE(this,2)'>";
					...
					echo "<tr>
				             <strong>
				               <td colspan='2' align='right'>TOTAL COMPLETOS: </td><td></td>
						   ...
						   <tr><td colspan='7' align='center'>Piezas Sueltas (Cajas Multiples)</td></tr>";
