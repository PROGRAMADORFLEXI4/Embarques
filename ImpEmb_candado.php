<?php
	...
	elseif(trim(isset($_GET['fac2']))<>"")
	{
		...
		$numero_pedido=$dPed['ZZO_PEDIDO'];
		$codigo_cliente=$dPed['ZZO_CLTE'];
		odbc_free_result($sql);
		
		// aqui se hara la validacion de los pedidos que vienen atras para ver si se tiene que esperar
		$cuantos=0;
		$numeros_pedidos="";
		$sql=odbc_exec($conn,"SELECT C5_NUM FROM (SELECT C5_NUM FROM SC5010 WHERE D_E_L_E_T_='' AND C5_NUM<>'".$numero_pedido."' AND C5_CLIENTE='E00003' AND C5_DIREMB='FISCAL' AND C5_FYHRCYC<>'' AND  C5_NOTA='') C5
											LEFT JOIN (SELECT R_E_C_N_O_,ZZO_PEDIDO FROM ZZO010 WHERE D_E_L_E_T_='' AND ZZO_CLTE='E00003'  AND ZZO_CHOFER='' AND ZZO_FECHEN='' AND ZZO_PEDIDO<>'078215') ZZO ON C5_NUM=ZZO_PEDIDO")or die("Error al obtener el pedido");
		while($dPed=odbc_fetch_array($sql))
		{
			$cuantos++;
			if($numeros_pedidos!="")
				$numeros_pedidos.=", ";
			$numeros_pedidos.=$dPed['ZZO_PEDIDO'];
		}
		odbc_free_result($sql);
		...
		if($continuar)
		{
			if(trim($_GET['suc'])=="FISCAL")
				$sql=odbc_exec($conn,"SELECT A1_MUN AS 'destino' FROM SA1010 WHERE A1_COD='$codigo_cliente' AND D_E_L_E_T_=''");
			else
				$sql=odbc_exec($conn,"SELECT ZD1_POBLAC AS 'destino' FROM ZD1010 WHERE ZD1_CLIENT='$codigo_cliente' AND ZD1_CLAVE='$_GET[suc]' AND D_E_L_E_T_=''");
			$dPed=odbc_fetch_array($sql);
			odbc_free_result($sql);
			//
				...
				<!DOCTYPE html><html>
					...
					<body class='body'>
						<div class='page1'>
						   ...
						   <div class='pP'>
							   <img src='images/bannersobre.png' height='65' width='1000'>
						   </div>
						</div>
					</body>
				</html>";
