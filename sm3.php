...
/*			echo "<script>window.close();</script>";*/
	...
	$sql=odbc_exec($conn,"SELECT CP_FYHAP,CP_FYHLIM,CP_SOLICIT,CP_CTEEMB,CP_DIREMB FROM SCP010 WHERE CP_NUM='".substr($_POST['np'],2)."' AND D_E_L_E_T_=''  GROUP BY CP_FYHAP,CP_FYHLIM,CP_SOLICIT,CP_CTEEMB,CP_DIREMB")or die("Error SA");
	$datos=odbc_fetch_array($sql);
	odbc_free_result($sql);
...
<?php
	try{
		if(isset($_POST['np'])){
			...
			$sql=odbc_exec($conn,"SELECT CP_PRODUTO,CP_DESCRI,CP_QUANT,CP_QUJE,B1_UM,B1_SEGUM,B1_CONV,CASE WHEN B1_CONV<>0 THEN CP_QUANT*B1_CONV ELSE 0 END CP_QUANT2,CASE WHEN B1_CONV<>0 THEN CP_QUJE*B1_CONV ELSE 0 END CP_QUJE2 FROM SCP010 CP INNER JOIN SB1010 B1 ON CP_PRODUTO=B1_COD AND B1.D_E_L_E_T_='' WHERE CP_NUM='".substr($_POST['np'],2)."' AND CP.D_E_L_E_T_='' ORDER BY CP_PRODUTO")or die("Erro MP");
