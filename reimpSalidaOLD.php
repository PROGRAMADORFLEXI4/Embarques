					...
					while($impreso<2)
					{
						...
						if(odbc_num_rows($sql)>0)
						{
							...
							echo "<table border='1'>
								...
								odbc_free_result($sql);
								$sql=odbc_exec($conn,"SELECT ZDS_QTENT,B1_COD,B1_DESC,ZDS_PEDIM,B1_TIPO,B1_CLASE,B1_BOLSA,ZDS_COSTAL,ZDS_QE,ZDS_CAJA,ZDS_PARTID FROM ZDS010 ZDS INNER JOIN SB1010 SB1 ON ZDS_PRODUC=B1_COD WHERE ZDS_SALIDA=".$sal." AND SB1.D_E_L_E_T_='' AND ZDS.D_E_L_E_T_='' ORDER BY ZDS_CAJA,ZDS_PARTID,ZDS.R_E_C_N_O_")
									...
									echo "<tr>
									  ...
									  while($datos=odbc_fetch_array($sql))
									  {
										if($datos["ZDS_CAJA"]>0)
										{
											...
											if($multiBox==0)
											{
												echo "<tr borderColor='#FFF'><td colspan='7' align='center'>CAJAS MULTIPLES</td></tr>";
												$lineas+=1;
												$multiBox=1;
											}
										}
										...
										echo "<tr bgColor='#FFF' borderColor='#FFF'>
												<td align='right' style='border-bottom-color:#CCC'>".intval($datos["ZDS_QTENT"])."</td>
												<td style='border-bottom-color:#CCC'>[".trim($datos["B1_COD"])."] ".trim($datos["B1_DESC"])."</td>
												<td style='border-bottom-color:#CCC'>".trim($datos["ZDS_PEDIM"])."&nbsp;</td>";
										if($partida<>$datos["ZDS_PARTID"] || $partida==0)
										{
											...
											{
												$exhi+=$cantidad;
												echo "<td style='border-bottom-color:#CCC'>&nbsp;</td>";
											}
											elseif(trim($datos["B1_BOLSA"])=="T" || $datos["ZDS_COSTAL"]=="T")
											{
												$costal+=$cantidad;
												echo "<td style='border-bottom-color:#CCC'>&nbsp;</td><td style='border-bottom-color:#CCC'>&nbsp;</td>";
											}
											else
											{
												$caja+=$cantidad;
												echo "<td style='border-bottom-color:#CCC'>&nbsp;</td><td style='border-bottom-color:#CCC'>&nbsp;</td><td style='border-bottom-color:#CCC'>&nbsp;</td>";
											}
											if($cantidad==0 && $agrupaBox<>$datos["ZDS_CAJA"])
											{
												$agrupaBox=$datos["ZDS_CAJA"];
												echo "<td align='right' style='border-bottom-color:#CCC'>1</td></tr>";
												if(trim($datos["B1_CLASE"])=="11")
													$tinaco+=1;
												elseif(trim($datos["B1_CLASE"])=="22" && $datos['ZDS_QE']>0)
													$exhi+=1;
											...
											elseif($cantidad<>0)
												echo "<td align='right' style='border-bottom-color:#CCC'>".$cantidad."</td></tr>";
											else
											{
								...
								echo "<tr borderColor='#FFF'>
										<strong>
										   <td colspan='2' align='right'>TOTAL: </td><td></td>
										   <td align='right' style='border-top-color:#CCC'><input type='hidden' name='tinaco' value='".$tinaco."'>".$tinaco."</td>
										   <td align='right' style='border-top-color:#CCC'><input type='hidden' name='exhi' value='".$exhi."'>".$exhi."</td>
										   <td align='right' style='border-top-color:#CCC'><input type='hidden' name='costal' value='".$costal."'>".$costal."</td>
										   <td align='right' style='border-top-color:#CCC'><input type='hidden' name='caja' value='".$caja."'>".$caja."</td>
										 </strong>
									  </tr>";
									  ...
									  echo  "<tr bgcolor='#CCCCCC'>
											   <td colspan='2'>
												<table>
													...
													<tr>
														<td><input type='hidden' name='txtCodAlm' value='".$datos["ZZN_CODIGO"]."'><strong>Nombre:&nbsp;&nbsp;".trim($datos["ZZN_NOMBRE"])."</strong></td>
													</tr>
													<tr>
