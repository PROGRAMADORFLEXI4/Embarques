  ...
  <head>
	<title></title>
		<script language='javascript'>
			/*function imprimeSalida(ped,alm,sal){*/
			function imprimeSalida(orden_sur,alm,sal){
				window.print();
				window.open('index.php', '_self');
				//window.location.href ="pedido_Reisalida.php?pedido=" + ped + "&salida=" + sal;
			}
		</script>
...
-->
    ...
    <?php
	  ...
	  /*$sql=odbc_exec($conn,"SELECT ZZS_SALIDA,ZZS_CODALM,ZZS_FYHSAL FROM ZZS010 WHERE ZZS_PEDIDO='".$_GET['Ped']."' AND D_E_L_E_T_='' ORDER BY ZZS_SALIDA DESC") or die("Error al obtener los datos de la salida");*/
	  $sql=odbc_exec($conn,"SELECT ZZS_SALIDA,ZZS_CODALM,ZZS_FYHSAL FROM ZZS010 WHERE ZZS_ORDSUR='".$_GET['ordsur']."' AND D_E_L_E_T_='' ORDER BY ZZS_SALIDA DESC") or die("Error al obtener los datos de la salida");
	  if(odbc_num_rows($sql)>0)
	  {
		  ...
		  $fechaSal=$datos['ZZS_FYHSAL'];
	 		  /*echo "<body onload=imprimeSalida('".$_GET['Ped']."','".$alm."','".$sal."');>*/
	  	  echo "<body onload=imprimeSalida('".$_GET['ordsur']."','".$alm."','".$sal."');>
		  		  <form><center>";
					...
					while($impreso<2)
					{
						...
						$multiBox=0;
						$partida=0;
						$sql=odbc_exec($conn,"SELECT C5_DIREMB,C5_NUM,C5_EMISSAO,A1_COD,A1_NOME,C5_FYHSURT FROM SC5010 SC5 INNER JOIN SA1010 SA1 ON C5_CLIENTE=A1_COD WHERE C5_NUM='".$_GET['Ped']."' AND SC5.D_E_L_E_T_='' AND SA1.D_E_L_E_T_=''") or die("Error al validar el pedido de Impresión en SC5");
						if(odbc_num_rows($sql)>0)
						{
							$datos=odbc_fetch_array($sql);
							echo "<table border='1'>
								<tr>
									...
									{
										echo "<img src='images/logo.png' height='40' width='80'>";
									}
									echo "
									</td>
									...
									if(trim($datos['A1_COD'])!="D00123")
									{
										echo "<strong>Fleximatic S.A. de C.V.</strong></font>";
									}
									echo "
									</td>
								...
								<tr>
									...
									if(trim($datos['A1_COD'])=="D00123")
									{
										...
										odbc_free_result($sql);	
										$nombre_cliente=trim($nombre["A1_NOME"]);
									}
								...
								echo "
									<!--<tr><td colspan='8'><hr></td></tr>-->
									  ...
									  $sql=odbc_exec($conn,"SELECT ZZN_CODIGO,ZZN_NOMBRE FROM ZZN010 ZZN INNER JOIN ZZM010 ZZM ON ZZN_CODIGO=ZZM_CODALM WHERE ZZM_ORDE
									  	='".$_GET['ordsur']."' AND ZZN.D_E_L_E_T_='' AND ZZM.D_E_L_E_T_=''")
