<?php
	...
	$sql=odbc_exec($conn,"SELECT ZPP_ADMIN,ZPP_LOCAL FROM ZPP010 WHERE ZPP_NOMPC='".strtoupper(gethostbyaddr($_SERVER['REMOTE_ADDR']))."' AND ZPP_PAGINA='Pedidos' AND D_E_L_E_T_=''")or die("Error al validar el equipo");
	$esAdmin="F";
...
<html>
 ...
 <form name="frmPedi2">
 	<header>
        ...
        <?php
            if($esAdmin=="T")
				echo "<label id='pPost' class='lblEmb'>Pedidos Postergados</label>
                        ...
                        <label class='btnMenu' id='btnIP'>Impresión de Pedido</label>
        ...
        ?>
        <table class="tablaH" style='widht:93%;'><tr><th class="thPed">Num. Pedido</th><th class="thCliente">Nombre del Cliente</th><th class="thPed">Fecha Ap. CYC</th><th class="thPed">Fecha de Surtido</th><th class="thPed">Fecha de Salida</th><th class="thPed">Fecha de Auditado</th><th class="thAlm">Almacenista</th><th class="thPed">Fecha L&iacute;mite</th><th class="thRes">Restan</th><th class="thInd">Ind</th></tr></table>
    </header>
    <main>
     <table cellpadding="0" cellspacing="0">
		...
		$exce=odbc_exec($conn,"SELECT C5_CLIENTE,C5_CONDPAG, C6_LOCAL,case when C5_CLIENTE = 'D00123' then 'pe' else  CASE WHEN C5_USERVTA='Merkadotecnia' THEN 'publicidad' ELSE CASE WHEN C5_USER='Atencion a Cliente' THEN 'aclaracion' ELSE CASE WHEN C5_FPROG>0 THEN CASE WHEN A1_TIPO=4 THEN 'exportacion' ELSE 'programados'  END ELSE CASE WHEN A1_TIPO=1 THEN 'vip' ELSE CASE WHEN A1_TIPO=2 THEN CASE WHEN C5_RECCLIE='SI' THEN 'rc' ELSE 'local' END ELSE CASE WHEN A1_TIPO=3 THEN 'foraneos' ELSE CASE WHEN A1_TIPO=4 THEN 'exportacion' ELSE CASE WHEN A1_TIPO=6 THEN 'cita' ELSE CASE WHEN A1_TIPO=8 THEN 'audit' ELSE 'especiales' END END END END END END END END END end AS 'tipo',CASE WHEN A1_TIPO=9 THEN 'cedis' ELSE '' END tipo2,C5_EMBCPED,ISNULL(ZVA_NUM,'') AS 'postergado',ISNULL(ZVA_OBSERV,'') AS 'desZVA',ISNULL(C5_OBSPED,'') AS 'obsPed',C5_NUM,SUBSTRING(A1_NOME,1,30) AS 'A1_NOME',C5_FYHRCYC,ISNULL(ZZM_FYHSUR,'') AS ZZM_FYHSUR,ISNULL(ZZM_CAJAS,0) AS ZZM_CAJAS,ISNULL(ZZM_FECSUR,'') AS ZZM_FECSUR,ISNULL(ZZM_FATURA,'') AS ZZM_FATURA,C5_URGENTE,ZZN_NOMBRE,ISNULL(ZZS_FYHSAL,'') AS ZZS_FYHSAL,DATEDIFF(mi,C5_FYHRCYC, C5_FYHSURT) AS 'horas',DATEDIFF(mi,C5_FYHSURT, GETDATE()) AS 'horasT',C5_FYHSURT FROM SC5010 SC5 INNER JOIN SC6010 SC6 ON C5_NUM=C6_NUM LEFT JOIN (SELECT ZZM_PEDIDO,ZZN_NOMBRE,ZZM_FYHSUR,(ZZM_CAJAS+ZZM_COSTAL+ZZM_EXHIB+ZZM_TINACO) AS ZZM_CAJAS,ZZM_FECSUR,ZZM_FATURA FROM ZZM010 ZZM LEFT JOIN ZZN010 ON ZZM_CODALM=ZZN_CODIGO WHERE ZZM.D_E_L_E_T_<>'*') AS ZZM ON C5_NUM=ZZM.ZZM_PEDIDO INNER JOIN SA1010 SA1 ON A1_COD=C5_CLIENTE LEFT JOIN ZZS010 ON C5_NUM=ZZS_PEDIDO LEFT JOIN SC9010 SC9 ON C5_NUM=C9_PEDIDO LEFT JOIN (SELECT ZVA_NUM,ZVA_OBSERV FROM ZVA010 WHERE ZVA_STATUS=1 AND D_E_L_E_T_='') AS ZVA ON C5_NUM=ZVA.ZVA_NUM WHERE C6_LOCAL='".$_SESSION["LOCAL"]."' AND C5_LOJAENT=A1_LOJA AND C5_FYHRCYC<>'' AND C5_NOTA='' AND (C6_TES='501' OR C6_TES='502' OR C6_TES='522' OR C6_TES='523') AND (C6_QTDVEN-C6_QTDENT)>0 AND C6_BLQ='' AND C9_BLCRED<>'09' AND C9_BLCRED<>'01' AND SC5.D_E_L_E_T_<>'*' AND SC9.D_E_L_E_T_<>'*' AND SC6.D_E_L_E_T_<>'*' AND SA1.D_E_L_E_T_<>'*' GROUP BY C5_NUM,c5_cliente,C5_OBSPED,C5_CONDPAG,A1_NOME,C5_FYHRCYC,ZZM_FYHSUR,ZZM_CAJAS,ZZM_FECSUR,ZZM_FATURA,C5_URGENTE,ZZN_NOMBRE,ZZS_FYHSAL,C5_FYHSURT,C5_RECCLIE,C5_EMBCPED,ZVA_NUM,ZVA_OBSERV,C5_USER,C5_USERVTA,A1_TIPO,C5_FPROG,C6_LOCAL ORDER BY CONVERT(DATETIME,C5_FYHRCYC),ZZM_FATURA ASC")or die("Error al ejecutar la consulta");
            $pedido="";	
            $pxs="";
            $ppa="";
            $pxf="";
            ...
            while($datos=odbc_fetch_array($exce))
            {
                if($pedido!=$datos["C5_NUM"])
                {
                    ...
                    if(trim($datos["ZZM_FATURA"])<>"")
                        $clrFuente='bko';
                    /*Valida si los pedidos tienen cajas surtidas significa que esta en espera de facturación o cuenta con bakorder*/
                    if($datos["ZZM_CAJAS"]>0 && trim($datos["ZZM_FATURA"])<>"")
                    {
                        $bkO.="
								<tr class='$clrFuente'>
									...
									<td class='thRes'>".($dias<=0 && $horas<=0 && ($minutos<=0 || $minutos>=60)?'Vencido':($dias>0?$dias.'D':'').($horas>0?$horas.'H':'').($minutos>0 && $minutos<60?$minutos.'M':''))."</td>
                    ...
                    elseif(trim($datos["ZZM_FYHSUR"])=="")
                    {
                        ...
                        echo "</td><td class='thCliente'>".$datos["A1_NOME"]."</td>";
                        echo "<td class='thPed'>".substr($datos["C5_FYHRCYC"],0,15)."</td><td class='thPed'>&nbsp;</td><td class='thPed'>&nbsp;</td><td class='thPed'>&nbsp;</td><td class='thAlm'>&nbsp;</td><td class='thPed'>".$datos["C5_FYHSURT"]."</td><td class='thRes'>".($dias<=0 && $horas<=0 && ($minutos<=0 || $minutos>=60)?'Vencido':($dias>0?$dias.'D':'').($horas>0?$horas.'H':'').($minutos>0 && $minutos<60?$minutos.'M':''))."</td><td class='thInd'>&nbsp;".$img."</td></tr>";
                    }
                    else
                    {
                        if($datos["ZZM_CAJAS"]==0)
                        {
                            ...
                            $pxs.="<tr class='$clrFuente'><td class='thPed'>";
                            ...
                            else
                            {
                                ...
                                {
                                    //Captura la salida
                                    $pxs.="<a class='$clrFuente' href='guardaTempo.php?Ped=".$datos["C5_NUM"]."&opc=N' target='_top' title='Captura Salida del Pedido: ".$datos["C5_NUM"]."'>".$datos["C5_NUM"]."</a>";
                                }
                            }
                            $pxs.="</td><td class='thCliente'>".$datos["A1_NOME"]."</td><td class='thPed'>".substr($datos["C5_FYHRCYC"],0,15)."</td><td class='thPed'>".substr($datos["ZZM_FYHSUR"],0,16)."</td><td class='thPed'>&nbsp;</td><td class='thPed'>&nbsp;</td><td class='thAlm'>".trim($datos["ZZN_NOMBRE"])."</td><td class='thPed'>".$datos["C5_FYHSURT"]."</td><td class='thRes'>".($dias<=0 && $horas<=0 && ($minutos<=0 || $minutos>=60)?'Vencido':($dias>0?$dias.'D':'').($horas>0?$horas.'H':'').($minutos>0 && $minutos<60?$minutos.'M':''))."</td><td class='thInd'>&nbsp;".$img."</td></tr>";
                        }
                        else
                        {
                            ...
                            if($esAdmin=="T"){
                                ...
                                if($datos["ZZM_FECSUR"]=="")
                                    $ppa.="<tr class='$clrFuente'><td class='thPed'><a class='$clrFuente' href='guardaTempo.php?Ped=".$datos["C5_NUM"]."&opc=N' target='_top' title='Captura Salida del Pedido: ".$datos["C5_NUM"]."'>".$datos["C5_NUM"]."</a>";
                                else
                                    $pxf.="<tr class='$clrFuente'><td class='thPed'>".$datos["C5_NUM"];
                            }

                            if($datos["ZZM_FECSUR"]==""){
                                $ppa.="</td><td class='thCliente'>".$datos["A1_NOME"]."</td><td class='thPed'>".substr($datos["C5_FYHRCYC"],0,15)."</td><td class='thPed'>".substr($datos["ZZM_FYHSUR"],0,16)."</td><td class='thPed'>".trim(substr($datos["ZZS_FYHSAL"],0,16))."</td><td class='thPed'>&nbsp;</td><td class='thAlm'>".trim($datos["ZZN_NOMBRE"])."</td><td class='thPed'>".$datos["C5_FYHSURT"]."</td><td class='thRes'>".($dias<=0 && $horas<=0 && ($minutos<=0 || $minutos>=60)?'Vencido':($dias>0?$dias.'D':'').($horas>0?$horas.'H':'').($minutos>0 && $minutos<60?$minutos.'M':''))."</td><td class='thInd'>&nbsp;".$img."</td></tr>";
                            }else{
                                $pxf.="</td><td class='thCliente'>".$datos["A1_NOME"]."</td><td class='thPed'>".substr($datos["C5_FYHRCYC"],0,15)."</td><td class='thPed'>".substr($datos["ZZM_FYHSUR"],0,16)."</td><td class='thPed'>".trim(substr($datos["ZZS_FYHSAL"],0,16))."</td><td class='thPed'>".$datos["ZZM_FECSUR"]."</td><td class='thAlm'>".trim($datos["ZZN_NOMBRE"])."</td><td class='thPed'>".$datos["C5_FYHSURT"]."</td><td class='thRes'>".($dias<=0 && $horas<=0 && ($minutos<=0 || $minutos>=60)?'Vencido':($dias>0?$dias.'D':'').($horas>0?$horas.'H':'').($minutos>0 && $minutos<60?$minutos.'M':''))."</td><td class='thInd'>&nbsp;".$img."</td></tr>";
                            }
                        }
                    }
                }
            ...
            //CONSULTA PARA PEDIDOS POR FACTURAR
            odbc_exec($conn,"SET LANGUAGE 'español';");

			$sql_ped_fac = "SELECT C5_NUM, C5_FYHRCYC, C5_FYHSURT, A1_NOME, ZZS_FYHSAL, ZZM_FYHSUR, ZZM_FECSUR, ZZN_NOMBRE, C6_LOCAL FROM SC5010 SC5 
							INNER JOIN SC6010 SC6 ON C5_NUM=C6_NUM 
							INNER JOIN SA1010 SA1 ON A1_COD=C5_CLIENT
							LEFT JOIN ZZS010 ON C5_NUM=ZZS_PEDIDO
							LEFT JOIN (SELECT ZZM_PEDIDO,ZZN_NOMBRE,ZZM_FYHSUR,(ZZM_CAJAS+ZZM_COSTAL+ZZM_EXHIB+ZZM_TINACO) AS ZZM_CAJAS,ZZM_FECSUR,ZZM_FATURA FROM ZZM010 ZZM 
							LEFT JOIN ZZN010 ON ZZM_CODALM=ZZN_CODIGO WHERE ZZM.D_E_L_E_T_<>'*') AS ZZM ON C5_NUM=ZZM.ZZM_PEDIDO 
							WHERE C6_LOCAL = '70' AND SC5.D_E_L_E_T_='' AND SC6.D_E_L_E_T_='' AND C5_NOTA = '' AND C5_FYHRCYC <> '' AND C6_BLQ='' AND (C6_TES='501' OR C6_TES='502' OR C6_TES='522' OR C6_TES='523') GROUP BY C5_NUM, C5_FYHRCYC, C5_FYHSURT, A1_NOME, ZZS_FYHSAL, ZZM_FYHSUR, ZZM_FECSUR, ZZN_NOMBRE, C6_LOCAL;";

			$sql_ped = odbc_exec($conn, $sql_ped_fac) or die("Error en consulta");
			while ($pedidos_ =  odbc_fetch_array($sql_ped)) {

			$fecha_resto = substr($pedidos_["C5_FYHSURT"],6,2)."-".substr($pedidos_["C5_FYHSURT"],3,2)."-".substr($pedidos_["C5_FYHSURT"],0,2).substr($pedidos_["C5_FYHSURT"],8);
			...
			if($esAdmin=="T"){
				$pxf .= "<td style='color: #ff3300;' class='thPed'>
						<img src='images/edit.png' title='Agregar Observaciones al pedido' onclick=javascript:window.open('guardaObsP.php?Ped=".$pedidos_['C5_NUM']."','_blank','resisable=no')>&nbsp;&nbsp;".$pedidos_['C5_NUM'];
			}else{
				$pxf .= "<tr><td style='color: #ff3300;' class='thPed'>".$pedidos_['C5_NUM'];
			}
			$pxf .= "</td><td style='color: #ff3300;' class='thCliente'>".$pedidos_['A1_NOME']."</td><td style='color: #ff3300;' class='thPed'>".substr($pedidos_['C5_FYHRCYC'],0,15)."</td><td style='color: #ff3300;' class='thPed'>".substr($pedidos_['ZZM_FYHSUR'],0,16)."</td><td style='color: #ff3300;' class='thPed'>".trim(substr($pedidos_['ZZS_FYHSAL'],0,16))."</td><td style='color: #ff3300;' class='thPed'>".$pedidos_['ZZM_FECSUR']."</td><td style='color: #ff3300;' class='thAlm'>".trim($pedidos_['ZZN_NOMBRE'])."</td><td style='color: #ff3300;' class='thPed'>".$pedidos_['C5_FYHSURT']."</td><td style='color: #ff3300;' class='thRes'>".($dias<=0 && $horas<=0 && ($minutos<=0 || $minutos>=60)?'Vencido':($dias>0?$dias.'D':'').($horas>0?$horas.'H':'').($minutos>0 && $minutos<60?$minutos.'M':''))."</td><td style='color: #ff3300;' class='thInd'>&nbsp; <img src='images/icon_new.png' title='CEDIS'/></td></tr>";
			}
          ?>
          </table>
         </td>
     ...
     <br/>
         <footer>
            Fleximatic S.A. de C.V.<br> Departamento de sistemas Mayo 2011<br>  Desarrollado por: Ing. Fernando Ju&aacute;rez, Lae. Ernesto Gonz&aacute;lez.    
        </footer>
     <br/><br/>
