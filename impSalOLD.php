		...
		while($impreso<2)
		{
			...
			if(odbc_num_rows($sql)>0)
			{
				...
				echo "<table border='1'>
						...
						echo "<tr>
						  ...
						  while($datos=odbc_fetch_array($sql))
						  {
							if($datos["ZDS_CAJA"]>0)
							{
								...
								if($multiBox==0)
								{
									echo "<tr><td colspan='7' align='center'>CAJAS MULTIPLES</td></tr>";
									$lineas+=1;
									$multiBox=1;
								}
							}
							...
							echo "<tr bgColor='#FFFFFF' borderColor='#FFFFFF'>
									<td align='right' style='border-bottom-color:#CCC'>".intval($datos["ZDS_QTENT"])."</td><td style='border-bottom-color:#CCC'>[".trim($datos["B1_COD"])."] ".trim($datos["B1_DESC"])."</td><td style='border-bottom-color:#CCC'>".trim($datos["ZDS_PEDIM"])."&nbsp;</td>";
					...
					odbc_free_result($sql);
					echo "<table width='100%' border='1' cellspacing='0'><tr borderColor='#FFFFFF'>
							<strong>
							   <td colspan='2' align='right'>TOTAL: </td><td></td>
							   <td align='right' style='border-top-color:#CCC'><input type='hidden' id='tinaco' value='".$tinaco."'>".$tinaco."</td>
							   <td align='right' style='border-top-color:#CCC'><input type='hidden' id='exhi' value='".$exhi."'>".$exhi."</td>
							   <td align='right' style='border-top-color:#CCC'><input type='hidden' id='costal' value='".$costal."'>".$costal."</td>
							   <td align='right' style='border-top-color:#CCC'><input type='hidden' id='caja' value='".$caja."'>".$caja."</td>
							 </strong>
						  </tr>";
						  ...
						  echo  "<tr bgcolor='#CCCCCC'>
								   <td colspan='2'>
									<table>
										...
										<tr>
											<td><input type='hidden' id='txtCodAlm' value='".$datos["ZZN_CODIGO"]."'><strong>Nombre:&nbsp;&nbsp;".trim($datos["ZZN_NOMBRE"])."</strong></td>
										</tr>
										<tr>
