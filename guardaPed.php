<?PHP
	...
	if(isset($_GET["emb"])==1){
		odbc_exec($conn,"UPDATE ZZO010 SET ZZO_OBSEMB='".$_POST["txtObs"]."' WHERE R_E_C_N_O_=".$_GET["regis"]." AND D_E_L_E_T_=''") or die("Error al actualizar las observaciones de embarques");
		echo "<script languaje='JavaScript'>
				window.close();
			  </script>";
	}elseif(isset($_GET["actPedi2"])=="T"){
		//Valida si la actualización es una observación en el Pedido
		odbc_exec($conn,"UPDATE SC5010 SET C5_OBSPED='".$_POST["txtObs"]."' WHERE C5_NUM='".$_POST["txtPed"]."' AND D_E_L_E_T_=''") or die("Error al actualizar las observaciones del Pedido");
		echo "<script languaje='JavaScript'>
				window.close();
			  </script>";
	}else{
		...
		//El código del almacenista no es valido
			...
			$sql=odbc_exec($conn,"
				SELECT 
					C6_TES,C6_ITEM,
					C6_PRODUTO, B1_CAJCER, 
					B1_DESC, B1_UBIC, 
					C6_QTDVEN-C6_QTDENT AS C6_QTDVEN, 
					C6_PRCVEN C6_PRUNIT,
					C6_VALOR, C6_PEDIM, 
					B1_QE, B1_CANTBOL, 
					(
						SELECT 
							CONVERT( VARCHAR(MAX), (SELECT RTRIM(Z43_NAVE)+'-'+RTRIM(Z43_TORRE)+'-'+RTRIM(Z43_NIVEL)+'-'+RTRIM(Z43_RACK) + '; ' 
						FROM 
							Z43010 
						WHERE Z43_COD = C6_PRODUTO FOR XML PATH('')))
					) 
					AS UBICAIONES 
				FROM 
					SC6010 SC6 
					INNER JOIN SB1010 SB1 ON B1_COD=C6_PRODUTO 
					INNER JOIN(SELECT C9_PRODUTO,C9_ITEM 
				FROM SC9010 
				WHERE C9_PEDIDO='".$_POST["txtPed"]."' AND C9_BLCRED<>'09' AND C9_BLCRED<>'10' AND D_E_L_E_T_='' 
				GROUP BY C9_PRODUTO,C9_ITEM) AS SC9 ON SC9.C9_PRODUTO=C6_PRODUTO AND SC9.C9_ITEM=C6_ITEM 
				WHERE C6_NUM='".$_POST["txtPed"]."' AND C6_QTDVEN-C6_QTDENT>0 AND C6_BLQ='' AND SC6.D_E_L_E_T_<>'*' AND SB1.D_E_L_E_T_<>'*' 
				ORDER BY C6_PRODUTO")or die("Error al ejecutar la consulta de partidas del pedido ".$_POST["txtPed"]);
			if(odbc_num_rows($sql)==0){
				echo "<script language='JavaScript'>
				...
				$ped=odbc_exec($conn,"
					SELECT 
						A1_TRANSP, A1_EST, 
						C5_OBSVTA, C5_APFLETE,
						C5_CLIENTE, A1_NOME, 
						C5_EMISSAO, A3_NOME, 
						CASE WHEN A1_EST='JAL' THEN 'Local' ELSE 'Foraneo' END AS est,
						A1_TIPO, C5_FYHRVTA,
						C5_FYHRCYC, C5_FYHSURT, 
						C5_FYHRIMP, C5_DIREMB, 
						A1_END, A1_BAIRRO, 
						A1_MUN+', '+A1_EST AS poblacion,
						A1_ENDENT,C5_EMBCPED 
					FROM 
						SC5010 SC5 
						INNER JOIN SA1010 SA1 ON A1_COD=C5_CLIENTE 
						LEFT JOIN SA3010 ON A3_COD=A1_VEND 
					WHERE 
						C5_LOJAENT=A1_LOJA AND 
						SC5.D_E_L_E_T_<>'*' AND 
						SA1.D_E_L_E_T_='' AND 
						C5_NUM='".$_POST["txtPed"]."';")or die("Error al ejecutar la consulta del pedido");
				$pedido=odbc_fetch_array($ped);
				 echo "<html>
									...
									$(document).on('ready', function(){ 
										$.ajax({
											...
											success: function (data) {
												...
												}else if(data.trim() == 'ERROR_ZZM'){
													alert('Ocurrio un error en el proceso del surtido del pedido (ZZM)');
													setTimeout(function () { 
														window.open('index.php', '_self');
							 						}, 1000);
													
												}else if(data.trim() == 'ERROR_Z77'){
													alert('Ocurrio un error en el proceso del surtido del pedido (Z77)');
													setTimeout(function () { 
														window.open('index.php', '_self');
							 						}, 1000);
													
												}else if(data.trim() == 'ERROR_SC5'){
													alert('Ocurrio un error en el proceso del surtido del pedido (SC5)');
													setTimeout(function () { 
														window.open('index.php', '_self');
							 						}, 1000);
													
												}else if(data.trim() == 'ERROR_YA_SURTIENDO'){
													alert('El pedido ya se encuentra en procreso de surtido, favor de verificar');
													setTimeout(function () { 
														window.open('index.php', '_self');
							 						}, 1000);
												}else{
													alert('Ocurrio un error en el proceso del surtido del pedido');
													setTimeout(function () { 
														window.open('index.php', '_self');
							 						}, 1000);
												}
						...
						<div id='dvAlm'>
							<h1>Se imprimio correctamente el Pedido?</h1>
							<div class='btnLinea'>
								<center>
									<button id='btnAcep'>Aceptar</button>
									<button id='btnCanc'>Cancelar</button>
									<button id='btnIm'>Imprimir</button>
								</center>
							</div>
						</div>
						<div class='page'>
							...
							echo "
							<tr>
								<th colspan='6' style='text-align:center;font-size:15px;font-weight:700;'><h1>".$leyendatxt."</h1></th>
							</tr>
							<tr>
							<th colspan='6' style='text-align:left'>Pedido: ".$_POST["txtPed"]." Cliente: ".$pedido["C5_CLIENTE"]." ".trim($pedido["A1_NOME"]).", Estado: ".$pedido["est"].", Vendedor: ".$pedido["A3_NOME"].", Tipo: ".$tipocte."</th>
							</tr>
						 ...
						 while($datos=odbc_fetch_array($sql)){
                            ...
                            if($datos['B1_CAJCER']=='T')
							{
							 	echo "<tr>
							 			...
							 			<td style='text-align:center;'><font face='times, serif' size='2'>".trim($datos["C6_TES"])."</font></td>
							 			<td><font face='times, serif' size='2'>".trim($datos["C6_PEDIM"])."</font></td>
							 			<td style='text-align:center;'><font face='times, serif' size='2'>$".number_format($datos["C6_PRUNIT"],2)."</font></td>
							 			<td style='text-align:center;'><font face='times, serif' size='2'>".number_format($datos["C6_QTDVEN"],0)."</font></td>
							 			<td class='tdD'><font face='times, serif' size='2'>".($cajas>0?$cajas."C ":"").($bolsas>0?$bolsas."B ":"").($resto>0?$resto."R ":"")."</font></td>
							 			<td style='text-align:center;'><font face='times, serif' size='2'>$".number_format($sub,2)."</font></td>
						 		...
						 		echo "<tr><td colspan='9'>Ubicaciones del producto: ".$datos["UBICAIONES"]."</td></tr>";
							 ...
							 else
							 {
							 	echo "<tr>
							 			...
							 			<td style='text-align:center;'>".trim($datos["C6_TES"])."</td>
							 			<td>".trim($datos["C6_PEDIM"])."</td>
							 			<td style='text-align:center;'>$".number_format($datos["C6_PRUNIT"],2)."</td>
							 			<td style='text-align:center;'>".number_format($datos["C6_QTDVEN"],0)."</td>
						 ...
						 odbc_free_result($sql);
						 echo "<tr><td colspan='8' style='text-align:right'>Total del pedido:</td><td  style='text-align:center'>$".number_format($valor,2)."</td></tr></table><strong>
